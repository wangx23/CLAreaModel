...
#'@import cluster
#'@import scclust
Clust_area_pw2 <- function(indexy, y, x, index_d, vardir,beta00,
                       nu = 1, gam = 3, lam = 0.5,
                       ...
                       method = "pam", k0 = 10)
{
  ...
  if(len_d == ncol(x))
  {
    ...
    for(j in 1:maxiter)
    {
      ...
      Wm <- diag(1/(nbar*(vardir + sig2_cur))) /mean(1/vardir)
      Xty <- t(Xm) %*% Wm %*% y
                              ...
                              seed = 1234, init_control = NULL, beta00 = NULL)
{
  ...
  loss_value0  <- rep(0, length(lamvec))
  ...
  if(len_d == 1)
  {
    init_control <- list(method ="pam")
  }


  if(is.null(beta00))
  {
    beta00 <- initialFUN(indexy, y, x, index_d,
                         method = init_control$method)
  }
  out = list()

  for(j in 1:length(lamvec))
  {
    resj <- Clust_area_pw2(indexy, y, x, index_d, vardir, beta00,
  ...
  res_df <- data.frame(lambda = lamvec,Khat = K_est, loss0 = loss_value0)

  outlist <- list(res_df = res_df, out = out)
  return(outlist)
}
